...
/**
 * Aliases mapping a canonical camelCase key to the legacy snake_case spelling(s)
 * it may still have on disk. Lets path navigation and format-preserving writes
 * work regardless of the spelling a file uses.
 *
 * `offset` is genuinely ambiguous across the model — a register uses
 * `address_offset`, a bit field uses `bit_offset` — so it lists both candidates.
 * `resolveKey` disambiguates using the object's OWN keys (whichever legacy
 * spelling is actually present on that node), not the path shape, so no path
 * context needs to be threaded through the walkers below. `width` only ever
 * appears on fields (`bit_width`), so it is unambiguous despite looking similar.
 */
const KEY_ALIASES: Record<string, string[]> = {
  ...
  offset: ['address_offset', 'bit_offset'],
  width: ['bit_width'],
};

/**
 * Resolve a path key against an object, falling back to whichever legacy alias
 * is actually present on that object if needed. Returns the key that actually
...
function resolveKey(obj: Record<string | number, unknown>, key: string | number): string | number {
  ...
  if (typeof key === 'string') {
    const candidates = KEY_ALIASES[key];
    if (candidates) {
