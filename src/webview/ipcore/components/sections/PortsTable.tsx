...
export const PortsTable: React.FC<PortsTableProps> = ({ ports: rawPorts, onUpdate }) => {
  ...
  const {
    ...
    getRowProps,
    getCellProps,
  } = useTableEditing<Port, (typeof COLUMN_KEYS)[number]>({
    rows: ports,
    columnKeys: COLUMN_KEYS,
    containerRef,
    createEmptyDraft: createEmptyPort,
    normalizeDraftForEdit: normalizePort,
    onCommit: (updated) => onUpdate(['ports'], updated),
  });
