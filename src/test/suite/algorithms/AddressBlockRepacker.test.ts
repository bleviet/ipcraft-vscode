...
describe('AddressBlockRepacker', () => {
  describe('repackBlocksForward', () => {
    it('should repack blocks forward maintaining sizes', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x0000, size: 0x1000 },
        { name: 'Block2', baseAddress: 0x2000, size: 0x1000 },
        { name: 'Block3', baseAddress: 0x5000, size: 0x2000 },
      ];

      const result = repackBlocksForward(blocks, 1);

      // Block 0 unchanged, blocks 1-2 repacked sequentially
      expect(result[0].baseAddress).toBe(0x0000);
      expect(result[1].baseAddress).toBe(0x1000); // After block 0
      expect(result[2].baseAddress).toBe(0x2000); // After block 1
    });

    it('should handle repacking from index 0', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x1000, size: 0x1000 },
        { name: 'Block2', baseAddress: 0x5000, size: 0x2000 },
      ];
      ...
      expect(result[1].baseAddress).toBe(0x1000);
    });

    it('should handle blocks with varying sizes', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x0000, size: 0x100 },
        { name: 'Block2', baseAddress: 0x1000, size: 0x2000 },
        { name: 'Block3', baseAddress: 0x5000, size: 0x500 },
      ];

      const result = repackBlocksForward(blocks, 1);

      expect(result[0].baseAddress).toBe(0x0000);
      expect(result[1].baseAddress).toBe(0x100); // 0x000 + 0x100
      expect(result[2].baseAddress).toBe(0x2100); // 0x100 + 0x2000
    });
    ...
    it('should preserve original array', () => {
      ...
      expect(blocks[1].baseAddress).toBe(0x2000);
      expect(result[1].baseAddress).toBe(0x1000);
    });

    it('should handle single block', () => {
      const blocks = [{ name: 'Block1', baseAddress: 0x5000, size: 0x1000 }];
    ...
    it('should handle type coercion for size property', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x0000, size: 0x1000 },
        { name: 'Block2', baseAddress: 0x2000, size: undefined as any },
      ];
  ...
  describe('repackBlocksBackward', () => {
    it('should repack blocks backward maintaining sizes', () => {
      ...
      expect(result[2].baseAddress).toBe(0x5000);
      expect(result[1].baseAddress).toBe(0x4000); // 0x5000 - 0x1000
      expect(result[0].baseAddress).toBe(0x3000); // 0x4000 - 0x1000
    });
    ...
    it('should handle repacking to end when fromIndex is last', () => {
      ...
      expect(result[1].baseAddress).toBe(0x1000);
      expect(result[0].baseAddress).toBeGreaterThanOrEqual(0);
    });

    it('should clamp to address 0', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x1000, size: 0x5000 }, // Too large
        { name: 'Block2', baseAddress: 0x2000, size: 0x1000 },
      ];
    ...
    it('should preserve original array', () => {
      ...
      // Original unchanged
      expect(blocks[0].baseAddress).toBe(0x0000);
    ...
    it('should handle single block', () => {
      ...
      expect(result[0].baseAddress).toBe(0x5000);
    });
  });
  ...
  describe('Edge cases', () => {
    it('should handle zero-sized blocks', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x0000, size: 0 },
        { name: 'Block2', baseAddress: 0x1000, size: 0x1000 },
      ];

      const forward = repackBlocksForward(blocks, 1);
      expect(forward[1].baseAddress).toBe(0); // After zero-sized block

      const backward = repackBlocksBackward(blocks, 0);
      expect(backward[0].baseAddress).toBeGreaterThanOrEqual(0);
    });

    it('should handle missing size property', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0x0000 } as any,
        { name: 'Block2', baseAddress: 0x1000, size: 0x1000 },
      ];

      const forward = repackBlocksForward(blocks as any, 0);
      expect(forward[0].baseAddress).toBe(0);
    });

    it('should handle very large addresses', () => {
      const blocks = [
        { name: 'Block1', baseAddress: 0xffff0000, size: 0x1000 },
        { name: 'Block2', baseAddress: 0xffff5000, size: 0x1000 },
      ];

      const result = repackBlocksForward(blocks, 1);
      expect(result[1].baseAddress).toBe(0xffff1000);
    });
