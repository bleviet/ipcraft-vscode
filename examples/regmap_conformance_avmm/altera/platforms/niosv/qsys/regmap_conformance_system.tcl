...
#
# Components: Nios V/m CPU, 32 KB on-chip memory, JTAG UART, System ID,
...
#
# Nios V and the JTAG-to-Avalon-MM debug master are combined in ONE system
...
# This resolves the plan's "Nios V + JTAG master: one system vs. two" open
# item in favor of one system.
#
# The Nios V C self-test (software/platform/niosv/main.c) and the System Console Tcl
...
set_instance_parameter_value niosv enableAvalonInterface {1}
set_instance_parameter_value niosv enableDebug {1}

# ── On-chip memory: 48 KB (instruction + data) ───────────────────────────────
add_instance onchip_mem altera_avalon_onchip_memory2
set_instance_parameter_value onchip_mem memorySize {49152}
...
# peek/poke without a Nios V firmware download -- the "install-free" path
...
# ── Avalon-MM data bus ────────────────────────────────────────────────────────
# Both Nios V and the JTAG debug master reach every slave; Platform Designer
...
add_connection niosv.data_manager        regmap_ctrl.S_AVMM
...
# ── IRQ: niosv.platform_irq_rx is the master (receiver), peripherals are senders ──────────
...
add_connection niosv.platform_irq_rx regmap_ctrl.irq_associated
set_connection_parameter_value niosv.platform_irq_rx/regmap_ctrl.irq_associated irqNumber {1}
add_connection niosv.platform_irq_rx regmap_ctrl.irq_no_bus
set_connection_parameter_value niosv.platform_irq_rx/regmap_ctrl.irq_no_bus irqNumber {2}

# ── Base address map ──────────────────────────────────────────────────────────
# regmap_conformance base is 0x00010000 -- both masters use the same absolute
# address for a given register (niosv.data_manager's C_REG_*_ADDR offsets in
# software/platform/niosv/main.c, and debug/conformance_sysconsole.tcl's
...
set_connection_parameter_value niosv.data_manager/regmap_ctrl.S_AVMM      baseAddress {0x00010000}
...
set_connection_parameter_value niosv.data_manager/niosv.dm_agent          baseAddress {0x00020000}
set_connection_parameter_value niosv.instruction_manager/niosv.dm_agent   baseAddress {0x00020000}

# ── Reset / exception vectors (both in on-chip RAM) ──────────────────────────
set_instance_parameter_value niosv resetSlave      {onchip_mem.s1}
...
set_interface_property led_external_connection EXPORT_OF regmap_ctrl.led

save_system regmap_conformance_system.qsys
puts "regmap_conformance_system.qsys saved (Nios V + JTAG-to-Avalon-MM debug master)"
