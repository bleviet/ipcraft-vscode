...
entity regmap_conformance_core is
  port (
    ...
    -- regmap_conformance.mm.yml). led(6 downto 0) is a steady (non-blinking)
    -- binary readout of TEST_PROGRESS.COUNT -- always lit, no gating -- and
    -- led(7) is a dedicated status LED that blinks slowly while the host
    -- runner is progressing and faster once TEST_PROGRESS.FAILED is set.
...
begin
  ...
  -- Hold the CPU interrupt request until software clears the W1C status bit.
  -- Platform Designer's Nios II IRQ mapper is level-sensitive and does not
  -- latch a one-clock event pulse on behalf of the source.
