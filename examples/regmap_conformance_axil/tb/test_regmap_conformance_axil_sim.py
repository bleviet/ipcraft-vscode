"""cocotb tests for regmap_conformance_axil, wrapped as individual pytest functions.

Each function here maps 1-to-1 to a @cocotb.test() in
regmap_conformance_axil_test.py, so VS Code's Testing panel and ``pytest tb/``
both show them as separate tests.

Run all tests:            pytest tb/
Run one test:              pytest tb/ -k test_slverr_on_unmapped_address
Use a different simulator: SIM=icarus pytest tb/

The HDL is compiled once by the ``sim_runner`` fixture in conftest.py.

Hand-written; managed: false in regmap_conformance_axil.ip.yml.
"""
