...
```
tb/results_test_register_access.xml
tb/results_test_reset_values.xml
```

These files are generated by the cocotb runner and contain pass/fail status plus any log messages captured during simulation. They are compatible with CI systems that consume JUnit XML (GitHub Actions, Jenkins, GitLab CI).
