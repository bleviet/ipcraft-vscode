...
| Byte-enable behavior | `daq_controller` partial `wr_strb`; generated AXI4-Lite `WSTRB` and Avalon-MM `byteenable` scenarios | Yes | Yes | Yes, register-file assertions and generated cocotb wrapper-level partial writes | Yes, AXI4-Lite `basic_peripheral` and Avalon-MM `comprehensive_avalon` synthesize | Yes, both comprehensive wrappers synthesize | No |
...
| AXI4-Lite register binding | `basic_peripheral`, `comprehensive_axi`, and `daq_controller` | Yes | Yes | Yes, generated cocotb checks reset, access semantics, partial writes, arrays, unmapped addresses, and deterministic randomized traffic | Yes, `basic_peripheral` | Yes, `comprehensive_axi` | No |
| Avalon-MM register binding | `comprehensive_avalon` | Yes | Yes | Yes, the same semantic oracle runs through the Avalon-MM adapter, including wait/valid handshakes | Yes, `comprehensive_avalon` | Yes, `comprehensive_avalon` | No |
...
- Register-level reset is checked by the generated manifest scoreboard when a
  generated fixture includes one.
...
| `src/test/integration/testbench.test.ts` | Generated cocotb semantic scoreboard for both HDL backends and both memory-mapped bus bindings |
